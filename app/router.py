...
from app.decision_mapper import OPERATIONS, validate_payload, dispatch
...
async def proxy_execute(request: Request, body: ProxyRequest):
    ...
    if body.operationType not in OPERATIONS:
    ...
    # Payload validation
    missing = validate_payload(body.operationType, body.payload)
    if missing:
    ...
    try:
        ...
        data, upstream_status, url = await dispatch(body.operationType, body.payload, provider)
